...
public function _do_call_GET_ALL_FILES($p, $vm) {
   ...
   if (count($p) <> 2) return $this->_RunTime_Exception("Wrong parameter count for GET ALL FILES(dir, ext)");
   try {
      $directory = $p[0];
	  $extension = $p[1];
...
public function _do_call_GET_RUN_Q($p, $vm) {
   if (count($p) > 1) return $this->_RunTime_Exception("Wrong parameter count for GET_RUN_Q([profile_item])");
   try {
      $a = hda_db::hdadb()->HDA_DB_pendingQ($p[0]);
      if (is_null($a)) {
...
public function _do_call_TRIGGER_PROCESS($p, $vm) {
   ...
   try {
      ...
      HDA_ReportTrigger($item, NULL, NULL, "code execute in ".$vm->ref_title());
	  $data_path = str_replace("\\","\\\\",$data_path);
		if (!is_null($code = hda_db::hdadb()->HDA_DB_addRunQ(
				...
				$note = "Added upload of {$data_filename} to pending process queue";
