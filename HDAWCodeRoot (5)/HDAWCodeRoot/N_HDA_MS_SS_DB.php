...
public function HDA_DB_AllRawRows($table) {
   ...
   return false;
	...
	public function HDA_DB_bestQEntry($from, $to) {
		...
		$qs = array();
		while ($row = $this->_fetch_row()) {
			$qs[$row['QueryLevel']] = $row['Nq'];
		}
		for ($q = $from; $q<=$to; $q++) {
			if (array_key_exists($q, $qs)==false) return $q;
		}
		return $from
	}
	...
	public function HDA_DB_inPendingQ($profile_item) {
		$query = "SELECT * FROM hda_monitor WHERE (ProfileItem ='{$profile_item}' ) ";
		if ($result = $this->_query($query)) {
