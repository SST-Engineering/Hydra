...
public function HDA_DB_AllRawRows($table) {
   ...
   return false;
	...
	public function HDA_DB_KeepLock() {
		$query = "UPDATE hda_lock SET LockDate=GetDate() WHERE SessionId='".session_id()."'";
		$this->_query($query);
		return true;
	}
	...
	public function HDA_DB_updateRcvFile($code, $rcvFile) {
		$query = "UPDATE hda_monitor SET RcvFile='{$rcvFile}' WHERE ItemId='{$code}' ";
		if (($result=$this->_query($query)) !== true) {
			HDA_SendErrorMail("Update monitor RcvFile ".print_r(sqlsrv_errors(),true)." {$query} ");
		}
		return true;
	}
	...
	public function HDA_DB_TaskComplete($code, $success=true) {
		$success = ($success)?1:0;
		$now = $this->PRO_DB_dateNow();
		$query = "UPDATE hda_monitor SET ProcessResult={$success},ProcessDate='{$now}',ProcessState='FINISHED'  WHERE ItemId='{$code}' ";
		if (($result = $this->_query($query))!==true) {
			HDA_SendErrorMail("Register Complete Task in monitor".print_r(sqlsrv_errors(),true)." {$query} ");
		}
		return true;
	}
	...
	public function HDA_DB_ReadTask($code) {
		$query = "SELECT * FROM hda_monitor WHERE ItemId='{$code}' ";
		if ($result = $this->_query($query)) {
			if ($row = $this->_fetch_row()) {
	...
	public function HDA_DB_addRunQ($code, $profileId, $user, $rcv_file, $rcv_file_name, $source, $source_info, $effective_date, $qLevel  = NULL) {
		...
		$rcv_file = _makeSQLstring($rcv_file);
		$rcv_file_name = _makeSQLstring($rcv_file_name);
		$source_info = _clean($source_info);
		$code = _clean($code);
		...
		'IssuedDate'=>$now,'EntryTime'=>$now,'Pulse'=>$now,'EffectiveDate'=>$effective_date, 'ProcessResult'=>0);
		$query = "IF EXISTS(SELECT ItemId FROM hda_monitor WHERE ItemId='{$code}') ";
		$query .= "UPDATE hda_monitor SET  ";
	...
	public function HDA_DB_bestQEntry($from, $to, $parent) {
		$query = "SELECT QueueLevel,COUNT(QueueLevel) as 'Nq' FROM hda_monitor WHERE QueueLevel>={$from} AND QueueLevel<={$to} GROUP BY QueueLevel ORDER BY 'Nq' ASC";
		if ($result = $this->_query($query)) {
			$qs = array();
			$jtot = 0;
			for ($q = $from; $q <=$to; $q++) $qs[$q] = 0;
			$rows = $this->_fetch_all();
			foreach ($rows as $row) {
				$qs[$row['QueueLevel']] = $row['Nq'];
				$jtot += $row['Nq'];
			}
			$tq = $from; $tn = 100; $m = "";
			for ($q = $from; $q<=$to; $q++) {
				if ($qs[$q] < $tn) {$tq = $q; $tn = $qs[$q];}
				$m .= " ({$q}): {$qs[$q]} ";
			}
//			$this->HDA_DB_issueNote($parent, "Best q {$tq} {$m}");
			file_put_contents("tmp/bestQ ".rand().date('Gis').".txt","{$tq} ".print_r($qs,true).print_r($rows,true).print_r(sqlsrv_errors(),true));
			return array($tq, $jtot);
		}
		HDA_SendErrorMail("Best Q ".print_r(sqlsrv_errors(),true)." {$query} ");
		return false;
	}
	public function HDA_DB_updatePending($code, $a) {
		...
		if ($result = $this->_query($query)) return true;
		HDA_SendErrorMail("Update run Q monitor ".print_r(sqlsrv_errors(),true)." {$query} ");
		return false;
	}


	public function HDA_DB_pendingQ($forProfileItem=NULL, $forEntry=NULL, $qLevel=NULL, $getRunnable=false) {
		$query = "SELECT  * FROM hda_monitor WHERE (ProfileItem IS NOT NULL) ";
		...
		HDA_SendErrorMail("Get run Q from monitor ".print_r(sqlsrv_errors(),true)." {$query} ");
		return NULL;
	}

	public function HDA_DB_inPendingQ($profile_item=NULL, $filename=NULL, $exclude=NULL) {
		$query = "SELECT * FROM hda_monitor WHERE ";
		if (!is_null($profile_item)) $query .= "(ProfileItem ='{$profile_item}' ) ";
		elseif (!is_null($filename)) $query .= "(RcvFileName = '{$filename}' ) AND (ProfileItem != '{$exclude}')";
		if ($result = $this->_query($query)) {
			if ($row = $this->_fetch_row()) {
				return ((is_null($row['ProcessState']))?'WAITING':$row['ProcessState']);
				}
			return false;
			}
	...
	public function HDA_DB_isDuplicateInQ($filename=NULL, $exclude=NULL) {
		$query = "SELECT * FROM hda_monitor WHERE ";
		$query .= "(RcvFileName = '{$filename}' ) AND (ProfileItem != '{$exclude}')";
		if ($result = $this->_query($query)) {
			if ($row = $this->_fetch_row()) {
				return true;
				}
		...
		HDA_SendErrorMail("Test dup in Q ".print_r(sqlsrv_errors(),true)." {$query} ");
		return false;
	}
	...
	public function HDA_DB_RunPending($item) {
		$query = "UPDATE hda_monitor SET ProcessState='RUNNING' WHERE ItemId='{$item}' ";
		if ($result = $this->_query($query)) return true;
	...
	public function HDA_DB_monitorRegister($item, $sessid, $pid=0) {
	   if (!is_numeric($pid)) $pid = 0;
	   $query = "UPDATE hda_monitor SET SessionId='{$sessid}',PID='{$pid}',Pulse=GetDate(),Mem=".memory_get_usage();
