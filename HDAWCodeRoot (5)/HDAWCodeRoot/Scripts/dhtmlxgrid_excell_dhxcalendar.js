...
eXcell_dhxCalendar.prototype = new eXcell;
	...
	eXcell_dhxCalendar.prototype.edit = function(){
                   ...
                   // alert(this.cell.val);
                    var t=this.grid._grid_calendarA.draw; this.grid._grid_calendarA.draw=function(){};
				   	this.grid._grid_calendarA.setDateFormat((this.grid._dtmask||"%Y-%m-%d %H:%i:00"));
	...
	eXcell_dhxCalendar.prototype.detach = function(){   
                    ...
                    if (this.grid._grid_calendarA._last_operation_calendar){
                    	var z1=this.grid._grid_calendarA.getFormatedDate((this.grid._dtmask||"%Y-%m-%d %H:%i:00"));
	...
	eXcell_dhxCalendar.prototype.setValue = function(val){
						...
						if (typeof val == "object"){
							...
							this.setCValue(this.grid._grid_calendarA.getFormatedDate((this.grid._dtmask||"%Y-%m-%d %H:%i:00"),val).toString(),this.cell.val);
							return;
						}
						...
						if(!val || val.toString()._dhx_trim()==""){
							...
							this.cell._clearCell=false;
							this.cell.val=new Date(this.grid._grid_calendarA.setFormatedDate((this.grid._dtmask_inc||this.grid._dtmask||"%Y-%m-%d %H:%i:00"),val.toString(),null,true));
							if (this.grid._dtmask_inc)
								val = this.grid._grid_calendarA.getFormatedDate((this.grid._dtmask||"%Y-%m-%d %H:%i:00"),this.cell.val);
						}
