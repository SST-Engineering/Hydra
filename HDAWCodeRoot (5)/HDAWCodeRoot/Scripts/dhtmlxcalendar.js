function dhtmlXCalendarObject(inps, skin) {
	...
	this.contDates.onclick = function(e){
		...
		if (t._date != null && !t._css_dis) {
			...
			var textBoxTarget = null;
			if (that._activeInp && that.i[that._activeInp]) {
			    textBoxTarget = that.i[that._activeInp].id;
			...
			//
			that.callEvent("onClick",[new Date(that._activeDate.getTime()), textBoxTarget]); // change TJ
	...
	this.setDateFormat("%Y-%m-%d %H:%i:00");
	...
	this._strToDate = function(val,getSet,format) {
		...
		for (var a in i) if (i[a] === false || isNaN(i[a])) i[a] = 0;
