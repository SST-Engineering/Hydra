...
class SMTP {
   ...
   * @return bool
   */
  public function Connect($host, $port = 0, $tval = 300) {
...
$this->smtp_conn = stream_socket_client("{$host}:{$port}",
   $errno, $errstr, 30, STREAM_CLIENT_CONNECT, $stream_context);
	...
	$retries = 0; $not_connected = empty($this->smtp_conn);
	while (($retries < 3) && ($not_connected)) {
    $this->smtp_conn = @fsockopen($host,    // the host of the server
                                 $port,    // the port to use
                                 $errno,   // error number if any
                                 $errstr,  // error message if any
                                 $tval);   // give up after ? secs
	$not_connected = empty($this->smtp_conn);
	$retries++;
	}
  ...
  public function Authenticate($username, $password, $authtype='LOGIN', $realm='', $workstation='') {
    ...
    switch ($authtype) {
      ...
      case 'NTLM':
         ...
         */
        require_once('../HDAWCodeRoot/Classes/PHPMailer_5.2.4/extras/ntlm_sasl_client.php');
        $temp = new stdClass();
        ...
        $rply = $this->get_lines();
        $code = substr($rply,0,3);
        $this->edebug("Will auth {$authtype} {$code} ".$this->CRLF);
  ...
  private function SendHello($hello, $host) {
    ...
    if($code != 250) {
      $this->error =
        array("error" => $hello . " " . $host . " not accepted from server",
              "smtp_code" => $code,
              "smtp_msg" => substr($rply,4));
      if($this->do_debug >= 1) {
        $this->edebug("SMTP -> ERROR: " . $this->error["error"] . ": " . $rply . $this->CRLF . '<br />');
