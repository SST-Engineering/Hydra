...
public function openSheet($sheet) {
	return $this->load($sheet);
	}
public function WriteTable($table, $start_row) {
	try {
		...
		file_put_contents("by_column_easyxl.txt", print_r($table,true));
		foreach ($table[0] as $field=>$value) {
			...
			$this->table->easy_setCellAt($cell, $row, $on_column);
			$on_column += 1;
		}
		$on_row++;
	...
	private function _make_good_text($value) {
		return $value;
		try {
	...
	public function getCell($pos) {
		try {
			...
			return $this->_make_good_text($nvalue);
	...
	public function setColumnStyle($column, $fore_color=null, $back_color=null, $border=null){
		try {
			$xlsStyleHeader = new COM("EasyXLS.ExcelStyle");
			$xlsStyleHeader->setFont("Verdana");
			$xlsStyleHeader->setFontSize(10);
			...
			$xlsStyleHeader->setWrap(false);
			//$xlsStyleHeader->setDataType($DATATYPE_STRING);
			$this->table->easy_getColumnAt($column)->setStyle($xlsStyleHeader);
			return true;
		}
		catch (Exception $r) {
			throw new Exception("Set Column Style ".$r);
		}
	...
	public function removeSheet($tabn) {
		try {
			$this->workbook->easy_removeSheet($tabn+1);
			}
		catch (Exception $r) {
	...
	public function removeRows($r1, $nRows) {
		try {
			...
			throw new Exception("Remove Row Range error {$r1} at {$nRows} ".$r);;
		}
	}
	public function getRow($row) {
		try {
			$r = $this->table->easy_getRowAt($row);
			/*if ($r->IsHiddenRow())*/ $r->setHiddenRow(false);
			return $r;
			}
		catch (Exception $r) {
			throw new Exception("Get Row error {$row}: ".$r);;
		}
	}
