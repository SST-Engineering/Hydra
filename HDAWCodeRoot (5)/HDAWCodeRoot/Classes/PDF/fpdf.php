...
function SetSubTitle($subtitle, $isUTF8=false)
{
	//Sub Title of document
	if($isUTF8)
		$subtitle=$this->_UTF8toUTF16($subtitle);
...
var $ReportTitle = "ActioNotes Report";
function ReportTitle($aTitle) {
...
function Footer()
{
   ...
   $this->SetX(0);
   $this->Cell(0,10,$this->ReportTitle);
   
   global $NowInt;
   $this->Cell(0, 10, alc_db::alcdb()->PRO_DBtime_Styledate($NowInt), 0, 0, 'R');
}
...
function SetDrawColor($ra, $g=null, $b=null)
{
      if (is_array($ra)) {
        $r = $ra[0]; $g=$ra[1]; $b=$ra[2];
        } else $r = $ra;
...
function SetFillColor($ra, $g=null, $b=null)
{
      if (is_array($ra)) {
        $r = $ra[0]; $g=$ra[1]; $b=$ra[2];
        } else $r = $ra;
...
function SetTextColor($ra, $g=null, $b=null)
{
      if (is_array($ra)) {
        $r = $ra[0]; $g=$ra[1]; $b=$ra[2];
        } else $r = $ra;
...
function ImageInfo($file, &$w, &$h, $type='') {
	//Get w and h
