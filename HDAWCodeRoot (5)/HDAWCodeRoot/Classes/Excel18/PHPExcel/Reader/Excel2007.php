				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					...
					if ($xmlWorkbook->sheets) {
						foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
							...
							// Check if sheet should be skipped
							if (isset($this->_loadSheetsOnly) && !$this->matchSheetNameInArray((string) $eleSheet["name"], $this->_loadSheetsOnly)) {
								++$countSkippedSheets;
							...
							//	file_put_contents("tmp/xlf.txt", print_r($this->getReadFilter(),true));
							//	file_put_contents("tmp/xl.txt", print_r($xmlSheet->sheetData,true));
